...
export default function Index(props) {
	...
	const filteredItems = data.filter(
		item => item.categorie && item.categorie.toLowerCase().includes(filterText.toLowerCase()),
	);

    const handleButtonDelete = (e, id) => {
        e.preventDefault();
        modalExcluded(async () => {
            const result = await axios.delete(route('category.destroy', id));
	...
	const fetchRows = async (page) => {
        ...
        const response = await axios.get(route('category.index', {json: true, page, per_page: perPage, term: filterText}));
	...
	const handlePerRowsChange = async (newPerPage, page) => {
        ...
        const response = await axios.get(route('category.index', {json: true, page, per_page: newPerPage, term: filterText}));
    ...
    const columns = [
        ...
        {		
            cell: (row) => <Link href={route('category.edit', row.id)}><BiPencil size={20} /></Link>,
            ignoreRowClick: true,
            allowOverflow: true,
            button: true,
        },
    ...
    return (
        <ClientScreen {...props}>
            ...
            <Link
                ...
                href={route('category.create')}>Nova Categoria
            </Link>
