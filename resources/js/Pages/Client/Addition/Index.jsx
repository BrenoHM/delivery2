...
export default function Index(props) {
	...
	const filteredItems = data.filter(
		item => item.addition && item.addition.toLowerCase().includes(filterText.toLowerCase()),
	);

    const handleButtonDelete = (e, id) => {
        e.preventDefault();
        modalExcluded(async () => {
            const result = await axios.delete(route('addition.destroy', id));
	...
	const fetchRows = async (page) => {
        ...
        const response = await axios.get(route('addition.index', {json: true, page, per_page: perPage, term: filterText}));
	...
	const handlePerRowsChange = async (newPerPage, page) => {
        ...
        const response = await axios.get(route('addition.index', {json: true, page, per_page: newPerPage, term: filterText}));
    ...
    const columns = [
        ...
        {		
            cell: (row) => <Link href={route('addition.edit', row.id)}><BiPencil size={20} /></Link>,
            ignoreRowClick: true,
            allowOverflow: true,
            button: true,
        },
    ...
    return (
        <ClientScreen {...props}>
            <Head title="Listagem de Acréscimos" />
            ...
            <Link
                ...
                href={route('addition.create')}>Novo Acréscimo
            </Link>
